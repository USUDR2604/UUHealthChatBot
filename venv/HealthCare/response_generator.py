...
context_mapping = {intent['tag']: intent['context'][0] if intent['context'] else "" for intent in intents['intents']}
...
def fetch_patient_details(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"_id": 0})
    if result:
        details = (
...
def fetch_medical_condition(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Medical Condition": 1, "_id": 0})
...
def fetch_medication(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Medication": 1, "_id": 0})
...
def fetch_billing_info(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Billing Amount": 1, "_id": 0})
...
def fetch_test_results(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Test Results": 1, "_id": 0})
...
def fetch_insurance_info(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Insurance Provider": 1, "_id": 0})
...
def fetch_admission_info(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Date of Admission": 1, "Admission Type": 1, "_id": 0})
    if result:
...
def fetch_discharge_info(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Discharge Date": 1, "_id": 0})
...
def fetch_blood_pressure(patient_id):
    result = patients_collection.find_one({"Patient ID": patient_id}, {"Blood Pressure": 1, "_id": 0})
...
def generate_response(user_input):
    ...
    tag_info = predicted_tag  # The predicted tag information

    # Obtain encoder and vector details for additional context
    encoder_info = encoder.inverse_transform([predicted_tag])[0]  # Decodes to human-readable label if encoded
    vector_info = input_vector.toarray()  # Represents vectorized user input

    # Example of handling a tag with specific responses (Modify as per actual requirements)
    if encoder_info == "greeting":
        response_text = random.choice(["Hello! How can I assist you today? 😊", "Hi there! What can I help you with today?"])
    ...
    # Further patient-specific handling can be added here with access to tag_info and encoder_info
